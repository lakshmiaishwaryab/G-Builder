...
const localStorageTransactions1 = JSON.parse(
    localStorage.getItem('transactions1')
);

let transactions1 =
    localStorage.getItem('transactions1') !== null ? localStorageTransactions : [];

// Add transaction
function addTransaction1(e) {
    e.preventDefault();

    if (text1.value.trim() === '' || date1.value.trim() === '') {
        alert('Please add a text and date');
    } else {
        const transaction1 = {
            id1: generateID1(),
            text1: text1.value,
        ...
        addTransactionDOM1(transaction1);
...
function addTransactionDOM1(transaction1) {
    
    const item1 = document.createElement('li');
    item1.innerHTML = `
    ${transaction1.text1} <span>${
        transaction1.date1
    }</span> <button class="delete-btn1" onclick="removeTransaction1(${transaction1.id1
        })">x</button>
  `;

    list1.appendChild(item1);
}
function removeTransaction1(id1) {
    transactions1 = transactions1.filter(transaction1 => transaction1.id1!== id1);
...
function init1() {
    list1.innerHTML = '';
    transactions1.forEach(addTransactionDOM1);
}

init1();

form1.addEventListener('submit', addTransaction1);
